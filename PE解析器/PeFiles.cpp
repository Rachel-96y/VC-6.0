// PeFiles.cpp: implementation of the PeFiles class.
//
//////////////////////////////////////////////////////////////////////
...
DWORD IsPeFile(IN LPTSTR lpszFilePath, IN PTLSDirectory g_pStTLS)
{
	/*
		检测文件是否是PE文件
	...
	*/
	...
	{
		// 修改全局PE标志位,根据32位结构体长度进行偏移
		FILE_OF_PE_BY_32(g_pStTLS -> m_dwGlobalFlag);
		fseek(lpszFileStream, 0x3A, SEEK_CUR);
	}
	else if (LOWORD(dwSignature) == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
	{
		// 修改全局PE标志位,根据64位结构体长度进行偏移
		FILE_OF_PE_BY_64(g_pStTLS -> m_dwGlobalFlag);
		fseek(lpszFileStream, 0x3E, SEEK_CUR);
	}
	else
	{
		// 非32位或64位程序
		return FILE_OF_PE_UNKNOWN;
	}
	...
	// 根据32位或64位分别定义指针
	if (FILE_OF_PE32_COMPARE(g_pStTLS -> m_dwGlobalFlag))
	{
		...
		// 将文件指针存储进结构体
		g_pStTLS -> m_fpFileBuffer = lpszFileBuffer;
	}
	else
	{
		// 定义DOS头结构体指针
