...
LPVOID CreateFileBuffer(IN LPSTR lpszFile)
{
	...
	*/
	...
	// 为此文件申请一块相同大小的空间
	LPVOID pFileBuffer = malloc(sizeof(CHAR) * (UINT)SizeOfFile + 0xA); 
	if (pFileBuffer == NULL)
	{
		printf("空间申请失败\n");
		return NULL;
	...
	// 将申请的空间内的数据初始化为 0
	memset(pFileBuffer, 0x00000000, sizeof(CHAR) * (UINT)SizeOfFile + 0xA);
...
DWORD MyAlignment(IN LPVOID pImageBuffer, IN UINT FileOrSectionAlignment,IN UINT SizeOfCode)
{
	/*
